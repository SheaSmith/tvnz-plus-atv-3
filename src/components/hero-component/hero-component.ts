...
export function expandRow(index: number, parentId: string, nextPage: string) {
    if (loading)
        return;
        ...
        snipeComponent.loadTemplate().then((c) => {
            try {
                ...
                getExpandedRow(nextPage, HeroComponentTemplate, (d) => {
                    ...
                    document.getElementById("hero").setAttribute("onItemFocused", d.getElementById("hero").getAttribute("onItemFocused"));
                    loading = false;
                });
            }
            catch (e) {
